...
* Providing the bot's functionality (Bloxburg alerts, player statistics).
* Diagnosing and fixing internal errors.
* Keeping basic metrics of server usage.

### 3. Data Removal and Retention

* When the bot leaves a server, its name and member count are automatically deleted.
...
If the privacy policy is updated, an embed will notify users the next time they use a command.
...
* Proveer las funcionalidades del bot (alertas de Bloxburg, estadísticas de jugadores).
...
* Cuando el bot abandona un servidor, se eliminan su nombre y cantidad de miembros.
...
Si la política de privacidad se actualiza, los usuarios serán notificados mediante un embed la próxima vez que usen un comando.
...
* Fornecer as funcionalidades do bot (alertas de Bloxburg, estatísticas de jogadores).
* Diagnosticar e corrigir erros internos.
* Manter métricas básicas do uso do bot nos servidores.

### 3. Remoção e Retenção de Dados

* Quando o bot sai de um servidor, seu nome e número de membros são automaticamente excluídos.
...
Se a política de privacidade for atualizada, um embed notificará os usuários na próxima vez que utilizarem um comando.
